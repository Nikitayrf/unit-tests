...
//        testingJavaCollectionsAssertJ(colors);
    ...
    public static void checkingShoppingCart() {
        ...
        productCategories.add( "water and juice" );
    ...
    public static void checkingHero() {
        ...
        5. Проверить, что герой это человек (свойство true)
        */
        assert emmett.getName().equals( "Emmett" );
        assert emmett.armorStrength == 50;
        assert emmett.getWeapon().equals( "sword" );
        assertThat( heroBag ).hasSize( 3 ).contains( "Bow", "Axe", "Gold" );
        assert emmett.isHuman == true;

    }
    public static void checkingCreationOfHero(Hero hero) {
        assert hero.getName().equals( "Emmett" );
    }
